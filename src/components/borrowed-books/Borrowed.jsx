...
const Borrowed = () => {
  ...
  useEffect( () => {
    ...
    axios
      .get(`http://localhost:4000/borrowedInfo?email=${user.email}`)
  ...
  borrowedInfo.forEach(borrowedBook => (
      borrowedBooks.push(allBooks.find(book => book._id == borrowedBook.bookID))
  ))
  ...
  console.log(borrowedBooks)
    ...
    const HandleDelete = (_id, bookID, quantity) => {
          console.log(_id)
          axios
          .delete(`http://localhost:4000/borrowedInfo?id=${_id}`)
          .then((deleteInfo) => {
    ...
    <div className="p-4 lg:px-24 min-h-screen">
      ...
      {borrowedBooks.length == 0 ? (
        ...
        borrowedBooks.map((each) => {
          const bookInfo = borrowedInfo.find(bookInfo => bookInfo.bookID == each?._id) 
          // const bookInfo = borrowedInfo.find(info => info.bookID === borrowedBook._id);
